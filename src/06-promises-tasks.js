...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((result, value) => {
    if (isPositiveAnswer) result('Hooray!!! She said "Yes"!');
    else if (isPositiveAnswer === false) result('Oh no, she said "No".');
    else value(new Error('Wrong parameter is passed! Ask her again.'));
  });
}
  ...
  return new Promise((exec) => {
    ...
    array.forEach((promise) => {
      promise.then((resolve) => {
        ...
        if (resolved.length === arrLen) exec(resolved.reduce(action));
      }).catch(() => {
        arrLen -= 1;
        if (resolved.length === arrLen) exec(resolved.reduce(action));
      });
